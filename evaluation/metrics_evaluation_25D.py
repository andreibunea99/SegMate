# evaluation/metrics_evaluation_25D.py
import os
import torch
...
def evaluate_model(
    ...
    if encoder_name is not None and model_class.__name__ in models_with_encoder:
        print(f"[metrics_evaluation_25D] Using encoder: {encoder_name}")
        base_model = model_class(num_classes=9, deep_supervision=deep_supervision, encoder_name=encoder_name).to(device)
    else:
        if encoder_name is not None:
            print(f"[metrics_evaluation_25D] Warning: encoder_name '{encoder_name}' ignored for {model_class.__name__}")
    ...
    print(f"\n🔍 Evaluating 2.5D on {split.upper()} set with threshold={threshold}")
        ...
        for batch in tqdm(dataloader):
            ...
            for i, organ in enumerate(ORGANS):
                ...
                try:
                    ...
                    if np.sum(p) > 0:
                        # NOTE: hd95 is computed in voxel (pixel) units here because
                        # the 2.5D dataloader does not expose per-patient voxel spacing.
                        # To get physically meaningful mm values, the dataloader would
                        # need to return per-slice spacing from the original NIfTI headers
                        # and it would be passed as: hd95(p, g, voxelspacing=(sy, sx)).
                        # Relative model rankings via HD95 are still valid since all models
                        # share the same evaluation data and spacing conditions.
    ...
    if device.type == "cuda":
        ...
        print(f"\n🧠 Peak VRAM usage during evaluation: {peak_mem_mb:.2f} MB")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--root_dir", type=str, default=ROOT_DIR, help="Dataset root dir (processed_dataset)")
