# evaluation/generate_amos22_report_25D.py
"""
Generate comprehensive model evaluation report for AMOS22 dataset (2.5D).
Adapted from generate_model_report-25D.py for AMOS22-specific evaluation.

AMOS22 Note:
-------------
- Dataset contains 5 overlapping organs: esophagus, liver, left_kidney, right_kidney, aorta
- Other 4 channels (lungs, cord, trachea) are zero-filled
- Metrics will be computed only for present organs
- Uses processed_dataset_amos22/ as default dataset path

Usage:
------
# Evaluate pre-trained TotalSegmentator model on AMOS22
python evaluation/generate_amos22_report_25D.py \
  --model checkpoints/segmate_film_tf_efficientnetv2_s_25D_best.pth \
  --model_type SegMateFiLM \
  --deep_supervision

# Evaluate fine-tuned AMOS22 model
python evaluation/generate_amos22_report_25D.py \
  --model checkpoints/segmate_amos22_finetune_best.pth \
  --model_type SegMateFiLM \
  --deep_supervision
"""
...
from evaluation.metrics_evaluation_amos22_25D import evaluate_model
...
# Keep all 9 organs for consistency (5 with data, 4 zero-filled)
ORGANS = [
...
def generate_report(model_path, deep_supervision=False, model_class=CustomUNetPlusPlus, encoder_name=None):
    ...
    logging.info(f"Dataset: AMOS22 (processed_dataset_amos22)")
    ...
    _, val_loader, test_loader = get_dataloaders("processed_dataset_amos22", batch_size=1, num_workers=2)
    val_size = len(val_loader.dataset)
    test_size = len(test_loader.dataset)

    logging.info("Running evaluation on AMOS22 validation set (2.5D)...")
    ...
    val_metrics = pd.read_csv("evaluation/metrics_summary_amos22_25D_val.csv")

    logging.info("Running evaluation on AMOS22 test set (2.5D)...")
    ...
    test_metrics = pd.read_csv("evaluation/metrics_summary_amos22_25D_test.csv")
    ...
    # Add 'finetuned' to report name if checkpoint has 'finetuned' in path
    suffix = "_finetuned" if "finetuned" in model_path.lower() else ""
    report_path = f"evaluation/amos22_model_report_25D{suffix}_{timestamp}.txt"

    with open(report_path, "w") as f:
        f.write("MODEL REPORT - AMOS22 Dataset (2.5D)\n")
        f.write("="*60 + "\n")
        f.write(f"Model Path: {model_path}\n")
        f.write(f"Dataset: AMOS22 (5 organs: esophagus, liver, left/right kidney, aorta)\n")
    ...
    try:
        ...
        from scripts.log_amos22_experiment import log_experiment
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Generate AMOS22 evaluation report (2.5D)"
    )
