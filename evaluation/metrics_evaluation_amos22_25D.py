# evaluation/metrics_evaluation_amos22_25D.py
"""
AMOS22-specific metrics evaluation (2.5D).
Adapted from metrics_evaluation_25D.py for AMOS22 dataset.
...
python evaluation/metrics_evaluation_amos22_25D.py \
  --split test \
  --model_path checkpoints/segmate_amos22_finetune_best.pth \
  --deep_supervision
"""
...
MODEL_PATH = "checkpoints/segmate_film_tf_efficientnetv2_s_25D_best.pth"
ROOT_DIR = "processed_dataset_amos22"  # AMOS22-specific
CSV_PATH = "evaluation/metrics_summary_amos22.csv"  # AMOS22-specific
...
def evaluate_model(
    ...
    if encoder_name is not None and model_class.__name__ in models_with_encoder:
        print(f"[metrics_evaluation_amos22_25D] Using encoder: {encoder_name}")
        base_model = model_class(num_classes=9, deep_supervision=deep_supervision, encoder_name=encoder_name).to(device)
    else:
        if encoder_name is not None:
            print(f"[metrics_evaluation_amos22_25D] Warning: encoder_name '{encoder_name}' ignored for {model_class.__name__}")
    ...
    print(f"\n[EVAL] Evaluating AMOS22 2.5D on {split.upper()} set with threshold={threshold}")
    ...
    print(f"\n[SAVED] AMOS22 summary to {csv_final}")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Evaluate AMOS22 dataset metrics (2.5D)"
    )
    ...
    parser.add_argument("--root_dir", type=str, default=ROOT_DIR, help="Dataset root dir (processed_dataset_amos22)")
