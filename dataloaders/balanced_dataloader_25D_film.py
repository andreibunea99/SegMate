...
"""
Balanced slice DataLoader (TotalSegmentator + chunked .npz support) - 2.5D with FiLM z_norm
============================================================================================
Based on balanced_dataloader_25D.py with added z_norm computation for FiLM conditioning.

Key change: Computes z_norm relative to organ presence boundaries, not raw slice indices.
This ensures consistency across different FOVs (full body, thorax, abdominal scans).

z_norm strategy:
- z_norm=0 -> first slice with any target organ
- z_norm=1 -> last slice with any target organ
"""
...
def get_dataloaders_balanced_film(root: str, batch_size: int = 60, num_workers: int = 8):
    """Create dataloaders that return (image, label, z_norm) tuples."""
