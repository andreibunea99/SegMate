"""
Train SegMateMambaOut (Thorax Segmentation) – fully instrumented -> 2.5D input
================================================================================
...
* Works on torchmetrics 1.7.1 specifically.
* Prints a concise device summary (CPU / all GPUs).
* Training checkpoint saving and resumption capability.
* DistributedDataParallel (DDP) support for multi-GPU training.

This script is based on train_segmate_fastvit_25D.py but uses MambaOut encoder.
MambaOut is a "Mamba without SSM" architecture that achieves competitive performance
without requiring the CUDA-dependent mamba-ssm library.

Available MambaOut variants:
- mambaout_tiny:  22.9M params (RECOMMENDED for starting)
- mambaout_small: 44.9M params
- mambaout_base:  79.4M params
...
python train/train_segmate_mambaout_25D.py \
  ...
  --logdir runs/segmate_mambaout25D

# Multi-GPU (DDP)
torchrun --nproc_per_node=3 train/train_segmate_mambaout_25D.py \
  --root processed_dataset --deep_supervision \
  --logdir runs/segmate_mambaout25D
...
torchrun --nproc_per_node=3 train/train_segmate_mambaout_25D.py \
  --root processed_dataset --deep_supervision \
  --logdir runs/segmate_mambaout25D \
  --resume checkpoints/segmate_mambaout_tiny_25D_latest.pth
"""
...
def build_parser():
    ...
    # Encoder selection (MambaOut variants)
    p.add_argument(
        "--encoder",
        type=str,
        default="mambaout_tiny",
        choices=["mambaout_tiny", "mambaout_small", "mambaout_base", "mambaout_kobe"],
        help="MambaOut encoder name from timm (default: mambaout_tiny)",
    )
    p.add_argument(
        "--no_pretrained",
        action="store_true",
...
def save_checkpoint(model, optimizer, scheduler, epoch, global_step, best_hd, cfg,
                    filename="segmate_mambaout_tiny_25D_latest.pth"):
...
def main(cfg):
    ...
    # ~~~~~~~~~~~~~ 2.5D fusion wrapper (3 slices -> 1 pseudo-slice) ~~~~~~~~~~~~~
    ...
    if is_main_process():
        print(f"\n{'='*60}")
        print(f"ENCODER INFO (MambaOut):")
        print(f"  Requested encoder: {cfg.encoder}")
        print(f"  Actual encoder type: {type(base_model.encoder).__name__}")
        try:
            print(f"  Encoder model name: {base_model.encoder.default_cfg.get('architecture', 'N/A')}")
        except AttributeError:
            print(f"  Encoder model name: {cfg.encoder}")
    ...
    for epoch in range(start_epoch, cfg.epochs + 1):
        ...
        if epoch % cfg.save_interval == 0:
            save_checkpoint(model, opt, sched, epoch, global_step, best_hd, cfg,
                            filename=f"segmate_{cfg.encoder}_25D_latest.pth")
        ...
        if mean_hd > best_hd:
            ...
            if is_main_process():
                Path("checkpoints").mkdir(exist_ok=True)
                checkpoint_path = f"checkpoints/segmate_{cfg.encoder}_25D_best.pth"
                ...
                save_checkpoint(model, opt, sched, epoch, global_step, best_hd, cfg,
                                filename=f"segmate_{cfg.encoder}_25D_best_resumable.pth")

    if is_main_process():
        log_header("TRAINING COMPLETE", 80)
        print(f"Best validation Dice: {best_hd:.4f}")
        print(f"Model saved to: checkpoints/segmate_{cfg.encoder}_25D_best.pth")
        print(f"Final checkpoint saved to: checkpoints/segmate_{cfg.encoder}_25D_latest.pth")
