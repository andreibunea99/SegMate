"""
Train SegMate FastViT + FiLM (Thorax Segmentation) - 2.5D input
================================================================================
Identical training setup to train_segmatev2_film_25D.py but uses
SegMateFastViTFiLM (FastViT encoder + FiLM conditioning).

Examples
--------
# Single GPU
python train/train_segmate_fastvit_film_25D.py \\
  --root processed_dataset --deep_supervision \\
  --logdir runs/segmate_fastvit_film

# Multi-GPU (DDP)
torchrun --nproc_per_node=3 train/train_segmate_fastvit_film_25D.py \\
  --root processed_dataset --deep_supervision \\
  --encoder fastvit_t12 \\
  --logdir runs/segmate_fastvit_film
...
torchrun --nproc_per_node=3 train/train_segmate_fastvit_film_25D.py \\
  --root processed_dataset --deep_supervision \\
  --logdir runs/segmate_fastvit_film \\
  --resume checkpoints/segmate_fastvit_film_fastvit_t12_25D_latest.pth
"""
...
def build_parser():
    ...
    p.add_argument("--logdir", default="runs/segmate_fastvit_film")
    ...
    p.add_argument("--encoder", type=str, default="fastvit_t12",
                   help="FastViT encoder variant (fastvit_t12, fastvit_s12, fastvit_sa12)")
...
def save_best_checkpoint(model, epoch, best_hd, encoder_name):
    ...
    ckpt_path = f"checkpoints/segmate_fastvit_film_{encoder_name}_25D_best.pth"
...
def save_checkpoint(model, optimizer, scheduler, epoch, global_step, best_hd, cfg):
    ...
    torch.save({
        ...
        "config": vars(cfg),
    }, f"checkpoints/segmate_fastvit_film_{cfg.encoder}_25D_latest.pth")
...
def main(cfg):
    ...
    if is_main_process():
        print(f"\n{'='*60}")
        print(f"SegMate FastViT FiLM Training")
    ...
    if is_main_process():
        ...
        print(f"Checkpoint: checkpoints/segmate_fastvit_film_{cfg.encoder}_25D_best.pth")
        if writer:
            writer.close()
