"""
Train SegMate with EfficientNetV2 + FiLM (Thorax Segmentation) - 2.5D input
================================================================================
* End-to-end training with deep-supervision and BalancedBatchSampler (2.5D loader).
* FiLM layer conditions bottleneck features on normalized slice position (z_norm).
...
--------
# Single GPU
python train/train_segmatev2_film_25D.py \
  --root processed_dataset --deep_supervision \
  --logdir runs/segmate_film

# Multi-GPU (DDP)
torchrun --nproc_per_node=3 train/train_segmatev2_film_25D.py \
  --root processed_dataset --deep_supervision \
  --encoder tf_efficientnetv2_m \
  --logdir runs/segmate_film_effv2m

# Resume
torchrun --nproc_per_node=3 train/train_segmatev2_film_25D.py \
  --root processed_dataset --deep_supervision \
  --logdir runs/segmate_film \
  --resume checkpoints/segmate_film_tf_efficientnetv2_s_25D_latest.pth
"""
...
def build_parser():
    ...
    # Encoder selection (EfficientNetV2 variants)
    p.add_argument(
        "--encoder",
        type=str,
        default="tf_efficientnetv2_s",
        help="EfficientNetV2 encoder from timm (tf_efficientnetv2_s, tf_efficientnetv2_m, tf_efficientnetv2_l)",
    )
    p.add_argument(
        "--no_pretrained",
        action="store_true",
        help="Disable ImageNet pretrained weights for the encoder",
    )

    # FiLM parameters
    p.add_argument("--film_hidden", type=int, default=128, help="FiLM MLP hidden dimension")
    p.add_argument("--film_layers", type=int, default=3, help="FiLM MLP number of layers")
    p.add_argument("--film_dropout", type=float, default=0.1, help="FiLM dropout rate")
    p.add_argument("--film_init_scale", type=float, default=0.01, help="FiLM weight initialization scale")
...
# ------------------- Custom Dice implementation (torchmetrics 1.7.1) ------------------
...
def save_checkpoint(model, optimizer, scheduler, epoch, global_step, best_hd, cfg,
                    filename="segmate_film_effv2_25D_latest.pth"):
...
def main(cfg):
    ...
    if is_main_process():
        ...
        print("Getting the DataLoaders (with z_norm for FiLM)")
    ...
    if is_main_process():
        print("Getting the model (SegMateFiLM)")

    # ~~~~~~~~~~~~~ 2.5D fusion wrapper (3 slices -> 1 pseudo-slice) with FiLM ~~~~~~~~~~~~~
    ...
    class TwoPointFiveDWrapperFiLM(nn.Module):
        """Wrapper that passes z_norm to the FiLM-enabled model."""
        ...
        def forward(self, x25d, z_norm=None):
            ...
            return self.model(x1, z_norm=z_norm)  # Pass z_norm to FiLM model

    # ~~~~~~~~~~~~~ Model + loss ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    base_model = SegMateFiLM(
        num_classes=len(ORGANS),
        in_channels=1,  # IMPORTANT: wrapper converts 3->1
        deep_supervision=cfg.deep_supervision,
    ...
    # Confirm encoder info
    if is_main_process():
        print(f"\n{'='*60}")
        print(f"ENCODER INFO:")
        print(f"  Requested encoder: {cfg.encoder}")
        print(f"  Actual encoder type: {type(base_model.encoder).__name__}")
        # Get architecture from default_cfg if available
        if hasattr(base_model.encoder, 'default_cfg'):
            arch = base_model.encoder.default_cfg.get('architecture', cfg.encoder)
        else:
            arch = cfg.encoder
        print(f"  Encoder model name: {arch}")
        enc_params = sum(p.numel() for p in base_model.encoder.parameters()) / 1e6
        total_params = sum(p.numel() for p in base_model.parameters()) / 1e6
        print(f"  Encoder params: {enc_params:.2f}M")
        print(f"  Total model params: {total_params:.2f}M")
        print(f"  Pretrained: {not cfg.no_pretrained}")
        print(f"\nFiLM CONFIGURATION:")
        print(f"  Hidden dim: {cfg.film_hidden}")
        print(f"  Num layers: {cfg.film_layers}")
        print(f"  Dropout: {cfg.film_dropout}")
        print(f"  Init scale: {cfg.film_init_scale}")
        print(f"{'='*60}\n")
    ...
    if is_main_process():
        log_header("TRAINING START (FiLM)", 80)
        print(f"Encoder: {cfg.encoder} (pretrained={'no' if cfg.no_pretrained else 'yes'})")
        print(f"FiLM: hidden={cfg.film_hidden}, layers={cfg.film_layers}, dropout={cfg.film_dropout}")
    ...
    for epoch in range(start_epoch, cfg.epochs + 1):
        ...
        for img, lbl, z_norm in pbar:  # Unpack 3 values (FiLM)
            img, lbl, z_norm = img.to(device), lbl.to(device), z_norm.to(device)

            opt.zero_grad(set_to_none=True)
            outputs = model(img, z_norm=z_norm)  # Pass z_norm to model
        ...
        with torch.no_grad():
            for img, lbl, z_norm in pbar:  # Unpack 3 values (FiLM)
                ...
                if cfg.deep_supervision and isinstance(outputs, tuple) and len(outputs) == 4:
                    deep_outs = outputs[3]
                    for i, ds_out in enumerate(deep_outs, 1):
                        ds_prob = torch.sigmoid(ds_out)
                        ds_hard = (ds_prob >= 0.5).float()
                        ds_dice = dice_hard(ds_hard, lbl)
                        vmeter[f"ds_{i}_dice"].update(ds_dice.mean().item(), img.size(0))
        ...
        if epoch % cfg.save_interval == 0:
            save_checkpoint(model, opt, sched, epoch, global_step, best_hd, cfg,
                            filename=f"segmate_film_{cfg.encoder}_25D_latest.pth")
        ...
        if mean_hd > best_hd:
            ...
            if is_main_process():
                Path("checkpoints").mkdir(exist_ok=True)
                checkpoint_path = f"checkpoints/segmate_film_{cfg.encoder}_25D_best.pth"
                ...
                # Also save resumable checkpoint
                save_checkpoint(model, opt, sched, epoch, global_step, best_hd, cfg,
                                filename=f"segmate_film_{cfg.encoder}_25D_best_resumable.pth")

    if is_main_process():
        log_header("TRAINING COMPLETE", 80)
        print(f"Best validation Dice: {best_hd:.4f}")
        print(f"Model saved to: checkpoints/segmate_film_{cfg.encoder}_25D_best.pth")
        print(f"Final checkpoint saved to: checkpoints/segmate_film_{cfg.encoder}_25D_latest.pth")
