# train_segmate_25D.py
"""
Train Custom UNet++ (10‑class Thorax Segmentation) – **fully instrumented** -> 2.5D input
===========================================================================
* End‑to‑end training with deep‑supervision and BalancedBatchSampler.
...
python train/train_custom_unetpp_logging_v2_with_resumption.py \
       --root processed_dataset --deep_supervision \
       --logdir runs/segmate25D_v2

# Start new training (multi-GPU with DDP)
torchrun --nproc_per_node=4 train/train_custom_unetpp_logging_v2_with_resumption.py \
       --root processed_dataset --deep_supervision \
       --logdir runs/segmate25D_v2
...
torchrun --nproc_per_node=4 train/train_custom_unetpp_logging_v2_with_resumption.py \
       --root processed_dataset --deep_supervision \
       --logdir runs/segmate25D_v2 --resume checkpoints/segmate25D_latest.pth
```
"""
...
import torchmetrics                # metrics lib – version may vary
...
def save_checkpoint(model, optimizer, scheduler, epoch, global_step, best_hd, cfg, 
                    filename='segmate25D_latest.pth'):
    """
...
def main(cfg):
    ...
    class TwoPointFiveDWrapper(nn.Module):
        def __init__(self, backbone_unetpp: nn.Module):
            super().__init__()
            self.fusion = SliceFusion(in_ch=3, mid_ch=16, out_ch=1)
            self.model = backbone_unetpp

        def forward(self, x25d):
            # x25d: [B, 3, H, W]  ->  x1: [B, 1, H, W]
            x1 = self.fusion(x25d)
            return self.model(x1)

    # ~~~~~~~~~~~~~ Model + loss ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    base_model = CustomUNetPlusPlus(
    ...
    # Experiment #5: No Presence Loss - dice=2.0, ce=1.0, boundary=0.1, presence=0.0
    ...
    for epoch in range(start_epoch, cfg.epochs + 1):
        ...
        if mean_hd > best_hd:
            ...
            if is_main_process():
                Path('checkpoints').mkdir(exist_ok=True)
                checkpoint_path = 'checkpoints/segmate25D_best.pth'
                ...
                save_checkpoint(model, opt, sched, epoch, global_step, best_hd, cfg, 
                              filename='segmate25D_best_resumable.pth')

    if is_main_process():
        log_header("TRAINING COMPLETE", 80)
        print(f"Best validation Dice: {best_hd:.4f}")
        print(f"Model saved to: checkpoints/segmate25D_best.pth")
        print(f"Final checkpoint saved to: checkpoints/segmate25D_latest.pth")
