# training_segmate_mambaout_nose.py
"""
Train SegMateMambaOutNoSE (Ablation Study - No SE on Skip Connections)
======================================================================
* Uses SegMateMambaOutNoSE model with MambaOut encoder
...
python train/training_segmate_mambaout_nose.py \
       --root processed_dataset --deep_supervision \
       --encoder mambaout_tiny --logdir runs/segmate_mambaout_nose

# Start new training (multi-GPU with DDP)
torchrun --nproc_per_node=4 train/training_segmate_mambaout_nose.py \
       --root processed_dataset --deep_supervision \
       --encoder mambaout_tiny --logdir runs/segmate_mambaout_nose
...
torchrun --nproc_per_node=4 train/training_segmate_mambaout_nose.py \
       --root processed_dataset --deep_supervision \
       --encoder mambaout_tiny --logdir runs/segmate_mambaout_nose --resume checkpoints/segmate_mambaout_nose_latest.pth
```
"""
...
def build_parser():
    ...
    p.add_argument("--logdir", default="runs/segmate_mambaout_nose")
    ...
    p.add_argument("--encoder", type=str, default="mambaout_tiny",
                   choices=["mambaout_tiny", "mambaout_small", "mambaout_base"],
                   help="MambaOut encoder name (default: mambaout_tiny)")
...
def save_checkpoint(model, optimizer, scheduler, epoch, global_step, best_hd, cfg,
                    filename='segmate_mambaout_nose_latest.pth'):
    """
    ...
    checkpoint = {
        ...
        'model_name': 'SegMateMambaOutNoSE'
    }
...
def main(cfg):
    ...
    if is_main_process():
        ...
        print("=" * 80)
        print(f" ABLATION STUDY: SegMateMambaOutNoSE with {cfg.encoder} ")
        print("=" * 80)
    ...
    if is_main_process():
        print(f"Getting the model: SegMateMambaOutNoSE with encoder {cfg.encoder}")

    # Model + loss
    model = SegMateMambaOutNoSE(
    ...
    if is_main_process():
        if start_epoch == 1:  # New training
            log_header("TRAINING START - SegMateMambaOutNoSE Ablation", 80)
    ...
    for epoch in range(start_epoch, cfg.epochs + 1):
        ...
        if mean_hd > best_hd:
            ...
            if is_main_process():
                Path('checkpoints').mkdir(exist_ok=True)
                checkpoint_path = 'checkpoints/segmate_mambaout_nose_best.pth'
                ...
                save_checkpoint(model, opt, sched, epoch, global_step, best_hd, cfg,
                              filename='segmate_mambaout_nose_best_resumable.pth')

    if is_main_process():
        log_header("TRAINING COMPLETE - SegMateMambaOutNoSE Ablation", 80)
        print(f"Best validation Dice: {best_hd:.4f}")
        print(f"Model saved to: checkpoints/segmate_mambaout_nose_best.pth")
        print(f"Final checkpoint saved to: checkpoints/segmate_mambaout_nose_latest.pth")
