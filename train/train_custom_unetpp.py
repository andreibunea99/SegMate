# train_custom_unetpp_logging_v2_with_resumption.py
"""
Train Custom UNet++ (10‑class Thorax Segmentation) – **fully instrumented**
===========================================================================
...
torchrun --nproc_per_node=4 train/train_custom_unetpp_logging_v2_with_resumption.py \
       --root processed_dataset --deep_supervision \
       --logdir runs/totalseg_v2 --resume checkpoints/totalseg_latest.pth
```
"""
...
def main(cfg):
    ...
    # ~~~~~~~~~~~~~ Model + loss ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    model = CustomUNetPlusPlus(num_classes=len(ORGANS), deep_supervision=cfg.deep_supervision).to(device)
    ...
    for epoch in range(start_epoch, cfg.epochs + 1):
        ...
        if mean_hd > best_hd:
            ...
            if is_main_process():
                Path('checkpoints').mkdir(exist_ok=True)
                checkpoint_path = 'checkpoints/totalseg_best.pth'
                ...
                save_checkpoint(model, opt, sched, epoch, global_step, best_hd, cfg, 
                              filename='totalseg_best_resumable.pth')

    if is_main_process():
        log_header("TRAINING COMPLETE", 80)
        print(f"Best validation Dice: {best_hd:.4f}")
        print(f"Model saved to: checkpoints/totalseg_best.pth")
        print(f"Final checkpoint saved to: checkpoints/totalseg_latest.pth")
