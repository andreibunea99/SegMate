"""
Train SegMateFastViT (Thorax Segmentation) – fully instrumented -> 2.5D input
================================================================================
...
This script is intentionally kept almost identical to your existing
train_segmate_25D.py / train_segmate_25D_v2 style, but swaps the backbone model
from CustomUNetPlusPlus -> SegMateFastViT (FastViT encoder via timm).
...
python train/train_segmate_fastvit_25D.py \
  ...
  --logdir runs/segmate_fastvit25D

# Multi-GPU (DDP)
torchrun --nproc_per_node=3 train/train_segmate_fastvit_25D.py \
  --root processed_dataset --deep_supervision \
  --logdir runs/segmate_fastvit25D
...
torchrun --nproc_per_node=3 train/train_segmate_fastvit_25D.py \
  --root processed_dataset --deep_supervision \
  --logdir runs/segmate_fastvit25D \
  --resume checkpoints/segmate_fastvit25D_latest.pth
"""
...
def build_parser():
    ...
    # Encoder selection (FastViT variants)
    p.add_argument(
        "--encoder",
        type=str,
        default="fastvit_t12",
        help="FastViT encoder name from timm (e.g., fastvit_t8, fastvit_t12, fastvit_sa24)",
    )
    p.add_argument(
        "--no_pretrained",
        action="store_true",
...
def save_checkpoint(model, optimizer, scheduler, epoch, global_step, best_hd, cfg,
                    filename="segmate_fastvit25D_latest.pth"):
    ...
    for epoch in range(start_epoch, cfg.epochs + 1):
        ...
        if epoch % cfg.save_interval == 0:
            save_checkpoint(model, opt, sched, epoch, global_step, best_hd, cfg,
                            filename=f"vanilla_segmate_{cfg.encoder}_25D_latest.pth")
        ...
        if mean_hd > best_hd:
            ...
            if is_main_process():
                Path("checkpoints").mkdir(exist_ok=True)
                checkpoint_path = f"checkpoints/vanilla_segmate_{cfg.encoder}_25D_best.pth"
                ...
                save_checkpoint(model, opt, sched, epoch, global_step, best_hd, cfg,
                                filename=f"vanilla_segmate_{cfg.encoder}_25D_best_resumable.pth")

    if is_main_process():
        log_header("TRAINING COMPLETE", 80)
        print(f"Best validation Dice: {best_hd:.4f}")
        print(f"Model saved to: checkpoints/vanilla_segmate_{cfg.encoder}_25D_best.pth")
        print(f"Final checkpoint saved to: checkpoints/vanilla_segmate_{cfg.encoder}_25D_latest.pth")
