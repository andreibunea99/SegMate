# training_segmate_fastvit_nose.py
"""
Train SegMateFastViTNoSE (Ablation Study - No SE on Skip Connections)
=====================================================================
* Uses SegMateFastViTNoSE model with FastViT encoder
...
python train/training_segmate_fastvit_nose.py \
       --root processed_dataset --deep_supervision \
       --encoder fastvit_t12 --logdir runs/segmate_fastvit_nose

# Start new training (multi-GPU with DDP)
torchrun --nproc_per_node=4 train/training_segmate_fastvit_nose.py \
       --root processed_dataset --deep_supervision \
       --encoder fastvit_t12 --logdir runs/segmate_fastvit_nose
...
torchrun --nproc_per_node=4 train/training_segmate_fastvit_nose.py \
       --root processed_dataset --deep_supervision \
       --encoder fastvit_t12 --logdir runs/segmate_fastvit_nose --resume checkpoints/segmate_fastvit_nose_latest.pth
```
"""
...
sys.path.append(".")
from models.segmate_fastvit_nose import SegMateFastViTNoSE
...
def build_parser():
    ...
    p.add_argument("--logdir", default="runs/segmate_fastvit_nose")
    ...
    # Encoder selection
    p.add_argument("--encoder", type=str, default="fastvit_t12",
                   choices=["fastvit_t8", "fastvit_t12", "fastvit_s12", "fastvit_sa12", "fastvit_sa24", "fastvit_sa36", "fastvit_ma36"],
                   help="FastViT encoder name (default: fastvit_t12)")
...
def save_checkpoint(model, optimizer, scheduler, epoch, global_step, best_hd, cfg,
                    filename='segmate_fastvit_nose_latest.pth'):
    """
...
def main(cfg):
    ...
    if is_main_process():
        ...
        print("=" * 80)
        print(f" ABLATION STUDY: SegMateFastViTNoSE with {cfg.encoder} ")
        print("=" * 80)
    ...
    if is_main_process():
        print(f"Getting the model: SegMateFastViTNoSE with encoder {cfg.encoder}")

    # Model + loss
    model = SegMateFastViTNoSE(
    ...
    if is_main_process():
        if start_epoch == 1:  # New training
            log_header("TRAINING START - SegMateFastViTNoSE Ablation", 80)
    ...
    for epoch in range(start_epoch, cfg.epochs + 1):
        ...
        if mean_hd > best_hd:
            ...
            if is_main_process():
                Path('checkpoints').mkdir(exist_ok=True)
                checkpoint_path = 'checkpoints/segmate_fastvit_nose_best.pth'
                ...
                save_checkpoint(model, opt, sched, epoch, global_step, best_hd, cfg,
                              filename='segmate_fastvit_nose_best_resumable.pth')

    if is_main_process():
        log_header("TRAINING COMPLETE - SegMateFastViTNoSE Ablation", 80)
        print(f"Best validation Dice: {best_hd:.4f}")
        print(f"Model saved to: checkpoints/segmate_fastvit_nose_best.pth")
        print(f"Final checkpoint saved to: checkpoints/segmate_fastvit_nose_latest.pth")
