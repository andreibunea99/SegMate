# training_segmate_nose.py
"""
Train SegMateNoSE (Ablation Study - No SE on Skip Connections)
==============================================================
* Exact replica of train_custom_unetpp.py but using SegMateNoSE model
...
python train/training_segmate_nose.py \
       --root processed_dataset --deep_supervision \
       --logdir runs/segmate_nose_ablation

# Start new training (multi-GPU with DDP)
torchrun --nproc_per_node=4 train/training_segmate_nose.py \
       --root processed_dataset --deep_supervision \
       --logdir runs/segmate_nose_ablation
...
torchrun --nproc_per_node=4 train/training_segmate_nose.py \
       --root processed_dataset --deep_supervision \
       --logdir runs/segmate_nose_ablation --resume checkpoints/segmate_nose_latest.pth
```
"""
...
def build_parser():
    ...
    p.add_argument("--logdir", default="runs/segmate_nose_ablation")
...
def save_checkpoint(model, optimizer, scheduler, epoch, global_step, best_hd, cfg,
                    filename='segmate_nose_latest.pth'):
    """
...
def main(cfg):
    ...
    if is_main_process():
        ...
        print("=" * 80)
        print(" ABLATION STUDY: SegMateNoSE (SE blocks removed from skip connections) ")
        print("=" * 80)
    ...
    if is_main_process():
        print("Getting the model: SegMateNoSE")

    # Model + loss
    model = SegMateNoSE(num_classes=len(ORGANS), deep_supervision=cfg.deep_supervision).to(device)
    ...
    if is_main_process():
        if start_epoch == 1:  # New training
            log_header("TRAINING START - SegMateNoSE Ablation", 80)
    ...
    for epoch in range(start_epoch, cfg.epochs + 1):
        ...
        if mean_hd > best_hd:
            ...
            if is_main_process():
                Path('checkpoints').mkdir(exist_ok=True)
                checkpoint_path = 'checkpoints/segmate_nose_best.pth'
                ...
                save_checkpoint(model, opt, sched, epoch, global_step, best_hd, cfg,
                              filename='segmate_nose_best_resumable.pth')

    if is_main_process():
        log_header("TRAINING COMPLETE - SegMateNoSE Ablation", 80)
        print(f"Best validation Dice: {best_hd:.4f}")
        print(f"Model saved to: checkpoints/segmate_nose_best.pth")
        print(f"Final checkpoint saved to: checkpoints/segmate_nose_latest.pth")
