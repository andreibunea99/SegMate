"""
Train SegMate with EfficientNetV2 (Thorax Segmentation) – 2.5D input
================================================================================
...
- tf_efficientnetv2_s (Small): ~24M params (default)
...
python train/train_segmatev2_25D.py \
  ...
  --logdir runs/segmate_effv2s

# Multi-GPU (DDP)
torchrun --nproc_per_node=3 train/train_segmatev2_25D.py \
  --root processed_dataset --deep_supervision \
...
torchrun --nproc_per_node=3 train/train_segmatev2_25D.py \
  --root processed_dataset --deep_supervision \
  --logdir runs/segmate_effv2s \
  --resume checkpoints/vanilla_segmate_tf_efficientnetv2_s_25D_latest.pth
"""
...
def save_checkpoint(model, optimizer, scheduler, epoch, global_step, best_hd, cfg,
                    filename="vanilla_segmate_effv2_25D_latest.pth"):
...
def main(cfg):
    ...
    # num_workers=2 for Windows compatibility (avoid memory allocation issues)
    train_ld, val_ld, _ = get_dataloaders(cfg.root, cfg.batch_size, num_workers=2)
