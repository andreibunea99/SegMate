# models/segmate_mambavision.py
"""
SegMate / CustomUNet++ variant with MambaVision encoder (NVIDIA, CVPR 2025)
===========================================================================
This is a COPY of your existing model (CBAM/SE/ASPP/Decoder/Dense skips/heads),
but replaces the EfficientNet-B5 encoder with a MambaVision encoder.

MambaVision is a hybrid Mamba-Transformer architecture from NVIDIA that achieves
state-of-the-art results on ImageNet while being efficient.

IMPORTANT: This model requires mamba-ssm which needs GLIBC >= 2.32.
           Run inside Singularity container if your system has older GLIBC.

MambaVision variants and their specs:
- mamba_vision_T (Tiny):  31.8M params, 82.3% ImageNet Top-1
- mamba_vision_S (Small): 50.1M params, 83.3% ImageNet Top-1
- mamba_vision_B (Base):  97.7M params, 84.2% ImageNet Top-1
- mamba_vision_L (Large): 227.9M params, 85.0% ImageNet Top-1

Requirements:
    pip install mamba-ssm mambavision

Usage:
    from models.segmate_mambavision import SegMateMambaVision
    model = SegMateMambaVision(num_classes=9, in_channels=1, deep_supervision=True, variant="T")
"""
...
try:
    from mambavision import create_model as create_mambavision
    MAMBAVISION_AVAILABLE = True
except ImportError:
    MAMBAVISION_AVAILABLE = False
    print("[WARNING] mambavision not available. Install with: pip install mambavision")
...
# MambaVision feature channels per variant
MAMBAVISION_CHANNELS = {
    "T": [80, 160, 320, 640],   # Tiny
    "S": [96, 192, 384, 768],   # Small
    "B": [128, 256, 512, 1024], # Base
    "L": [192, 384, 768, 1536], # Large
}


class MambaVisionEncoder(nn.Module):
    """
    Wrapper around MambaVision to extract hierarchical features.
    MambaVision doesn't have features_only mode, so we use forward hooks.
    """
    def __init__(self, variant="T", pretrained=True, in_channels=1):
        super().__init__()

        if not MAMBAVISION_AVAILABLE:
            raise ImportError("mambavision is not installed. Install with: pip install mambavision")

        model_name = f"mamba_vision_{variant}"
        print(f"[MambaVisionEncoder] Loading {model_name} (pretrained={pretrained})")

        self.model = create_mambavision(model_name, pretrained=pretrained)
        self.variant = variant
        self.channels = MAMBAVISION_CHANNELS[variant]

        # Input projection for 1-channel CT images
        if in_channels != 3:
        ...
        print(f"[MambaVisionEncoder] Encoder '{model_name}' loaded successfully")
        print(f"[MambaVisionEncoder] Feature channels: {self.channels}")
    ...
    def _register_hooks(self):
        ...
        # MambaVision has 4 levels
        for i, level in enumerate(self.model.levels):
            level.register_forward_hook(get_hook(i))

    def forward(self, x):
        # Clear previous features
        self.features = []

        # Project 1-channel to 3-channel if needed
        if self.input_proj is not None:
            x = self.input_proj(x)

        # Forward through model (hooks will capture features)
        _ = self.model(x)

        # Return the 4 stage features
        return self.features


class SegMateMambaVision(nn.Module):
    """
    Copy of your CustomUNetPlusPlus, but with MambaVision encoder.

    We keep the decoder/head channel plan the same as your EfficientNet-B5 version by aligning
    MambaVision feature maps to these target channels:
        e2 -> 40
        e3 -> 64
        e4 -> 128
        e5 -> 176
    ...
    def __init__(
        ...
        print(f"[SegMateMambaVision] Initializing with variant: MambaVision-{variant}")

        # ---- MambaVision encoder ----
        self.encoder = MambaVisionEncoder(variant=variant, pretrained=pretrained, in_channels=in_channels)

        # Get channel info for this variant
        enc_chs = MAMBAVISION_CHANNELS[variant]
        ch2, ch3, ch4, ch5 = enc_chs

        # ---- Align MambaVision stages to decoder-expected channels ----
    ...
    def forward(self, x):
        ...
        # MambaVision features: list of 4 feature maps
