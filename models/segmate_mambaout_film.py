# models/segmate_mambaout_film.py
"""
SegMate with MambaOut encoder + FiLM positional encoding.

Identical to SegMateFiLM (EfficientNetV2) but uses MambaOut as the encoder.
MambaOut has 4 stages (out_indices 0-3) and outputs NHWC format (channels last),
which requires permuting to NCHW before the decoder.

Usage:
    from models.segmate_mambaout_film import SegMateMambaOutFiLM
    model = SegMateMambaOutFiLM(num_classes=9, in_channels=1,
                                 encoder_name="mambaout_tiny", deep_supervision=True)
...
class SegMateMambaOutFiLM(nn.Module):
    """SegMate FiLM with MambaOut encoder (4-stage, NHWC → permuted to NCHW)."""
    ...
    def __init__(
        ...
        print(f"[SegMateMambaOutFiLM] Initializing encoder: {encoder_name}")
        # MambaOut has 4 stages → out_indices=(0,1,2,3)
        ...
        print(f"[SegMateMambaOutFiLM] Encoder '{encoder_name}' loaded")

        feat_info = self.encoder.feature_info
        enc_chs = [feat_info[i]["num_chs"] for i in range(4)]
        print(f"[SegMateMambaOutFiLM] Feature channels: {enc_chs}")
        ...
        print(f"[SegMateMambaOutFiLM] FiLM layer added (hidden={film_hidden_dim}, layers={film_num_layers})")
