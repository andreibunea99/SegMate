# models/segmate_mambaout.py
"""
SegMate / CustomUNet++ variant with MambaOut encoder (timm)
===========================================================
This is a COPY of your existing model (CBAM/SE/ASPP/Decoder/Dense skips/heads),
but replaces the EfficientNet-B5 encoder with a MambaOut encoder.
...
- We add alignment blocks that project MambaOut feature channels -> the SAME channel
  sizes your decoder expects (40, 64, 128, 176), so the rest of the network stays
  structurally consistent.

MambaOut variants and their feature channels:
- mambaout_tiny:  [96, 192, 384, 576]  - 22.9M params
- mambaout_small: [96, 192, 384, 576]  - 44.9M params
- mambaout_base:  [128, 256, 512, 768] - 79.4M params

NOTE: MambaOut outputs features in NHWC format (channels last), which we convert to NCHW.
...
Usage:
    ...
    model = SegMateMambaOut(num_classes=9, in_channels=1, deep_supervision=True, encoder_name="mambaout_tiny")
"""
...
class SegMateMambaOut(nn.Module):
    """
    Copy of your CustomUNetPlusPlus, but with MambaOut encoder.

    We keep the decoder/head channel plan the same as your EfficientNet-B5 version by aligning
    MambaOut feature maps to these target channels:
        e2 -> 40
        e3 -> 64
        e4 -> 128
        e5 -> 176
    ...
    MambaOut outputs features in NHWC format (channels last), which we permute to NCHW.
    """
    def __init__(
        self,
        num_classes=9,
        ...
        encoder_name: str = "mambaout_tiny",
        ...
        # ---- MambaOut encoder from timm (features_only returns list of feature maps) ----
        # in_chans lets you use 1-channel CT directly.
        print(f"[SegMateMambaOut] Initializing with encoder: {encoder_name}")
        ...
        print(f"[SegMateMambaOut] Encoder '{encoder_name}' loaded successfully")
        ...
        print(f"[SegMateMambaOut] Encoder feature channels: {enc_chs}")
        ...
        ch2, ch3, ch4, ch5 = enc_chs  # e2,e3,e4,e5 from MambaOut stages

        # ---- Align MambaOut stages to your decoder-expected channels (keep rest identical) ----
        ...
        # ASPP on deepest feature (MambaOut last stage channels -> 256)
    ...
    def forward(self, x):
        ...
        # MambaOut features: list of 4 feature maps
        # f2, f3, f4, f5 correspond to increasing depth / decreasing resolution
        features = self.encoder(x)

        # MambaOut outputs NHWC format, convert to NCHW
        # Shape: [B, H, W, C] -> [B, C, H, W]
        f2 = features[0].permute(0, 3, 1, 2).contiguous()
        f3 = features[1].permute(0, 3, 1, 2).contiguous()
        f4 = features[2].permute(0, 3, 1, 2).contiguous()
        f5 = features[3].permute(0, 3, 1, 2).contiguous()
