# models/vanilla_segmate_mambaout.py
"""
Vanilla SegMate / CustomUNet++ variant with MambaOut encoder (timm)
===================================================================
Same as your segmate_mambaout.py backbone swap (MambaOut encoder + alignment + ASPP + heads),
but **VANILLA**:
- NO CBAM
- NO SE

Everything else stays structurally identical:
- Decoder widths: 160 / 88 / 48 / 32
- Alignment targets: 40 / 64 / 128 / 176
- Same dense skip topology
- Same heads: segmentation / boundary / presence
- Optional deep supervision heads

NOTE: MambaOut outputs features in NHWC format (channels last), converted to NCHW.
...
# --- Decoder Block with Residual Connection (NO CBAM/SE) ---
...
class SegMateMambaOut(nn.Module):
    """
    Vanilla (no attention) SegMate with MambaOut encoder.

    Align MambaOut features to:
        e2 -> 40
        e3 -> 64
        e4 -> 128
        e5 -> 176
    Decode with same widths as your original.
    """
    def __init__(
        self,
        num_classes=9,
        ...
        print(f"[SegMateMambaOut(Vanilla)] Initializing with encoder: {encoder_name}")
        ...
        print(f"[SegMateMambaOut(Vanilla)] Encoder '{encoder_name}' loaded successfully")

        feat_info = self.encoder.feature_info
        enc_chs = [feat_info[i]["num_chs"] for i in range(len(out_indices))]
        print(f"[SegMateMambaOut(Vanilla)] Encoder feature channels: {enc_chs}")
        ...
        # Dense skip connections (NO SE)
        self.skip_dec3_4 = DecoderBlock(88 + 88, 88)
        self.skip_dec2_3 = DecoderBlock(48 + 48, 48)
        self.skip_dec1_2 = DecoderBlock(56, 32)          # keep identical choice
    ...
    def forward(self, x):
        ...
        # MambaOut outputs NHWC -> convert to NCHW
        ...
        # Dense skips (same topology, no attention)
        x1_3 = self.up3(x0_4)
