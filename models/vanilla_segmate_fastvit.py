# models/vanilla_segmate_fastvit.py
"""
VanillaSegMateFastViT - Baseline Model (No CBAM, No SE)
=======================================================
This is the FastViT-encoder variant of your SegMate/CustomUNet++ family,
but with ALL attention removed:
- No CBAM in decoder blocks
- No SE in dense skip blocks
Everything else is kept structurally identical to your SegMateFastViT layout:
- Same decoder widths (160/88/48/32)
- Same alignment targets (40/64/128/176)
- Same ASPP + 3 heads (segmentation/boundary/presence)
...
Usage:
    from models.vanilla_segmate_fastvit import VanillaSegMateFastViT
...
# --- ASPP Module (same as your codebase style) ---
...
# --- Feature Alignment Module (same as FastViT version) ---
...
# --- Decoder Block with Residual Connection (NO ATTENTION) ---
...
class SegMateFastViT(nn.Module):
    """
    Vanilla (no attention) version of SegMateFastViT.

    We keep downstream decoder/head plan identical by aligning FastViT feature maps to:
        e2 -> 40
        e3 -> 64
        e4 -> 128
        e5 -> 176
    Then decode with the same widths as your SegMateFastViT:
        256 -> up4 -> 160 -> up3 -> 88 -> up2 -> 48 -> up1 -> 32
    Dense skip blocks are also kept (but without SE).
    """
    def __init__(
        self,
        num_classes: int = 9,
        ...
        print(f"[VanillaSegMateFastViT] Initializing with encoder: {encoder_name}")
        ...
        print(f"[VanillaSegMateFastViT] Encoder '{encoder_name}' loaded successfully")
        ...
        ch2, ch3, ch4, ch5 = enc_chs  # f2,f3,f4,f5

        # Align to the same channels your decoder expects
        ...
        # ASPP on deepest feature (raw f5 channels -> 256)
        self.aspp = ASPP(ch5, 256)

        # Decoder path (NO CBAM)
        self.up4 = nn.ConvTranspose2d(256, 160, kernel_size=2, stride=2)
        self.dec4 = DecoderBlock(160 + 176, 160)

        self.up3 = nn.ConvTranspose2d(160, 88, kernel_size=2, stride=2)
        self.dec3 = DecoderBlock(88 + 128, 88)

        self.up2 = nn.ConvTranspose2d(88, 48, kernel_size=2, stride=2)
        self.dec2 = DecoderBlock(48 + 64, 48)

        self.up1 = nn.ConvTranspose2d(48, 24, kernel_size=2, stride=2)
        self.dec1 = DecoderBlock(24 + 40, 32)

        # Dense skip blocks (NO SE)
        self.skip_dec3_4 = DecoderBlock(88 + 88, 88)
        self.skip_dec2_3 = DecoderBlock(48 + 48, 48)
        self.skip_dec1_2 = DecoderBlock(56, 32)          # keep identical to your original choice
        self.skip_dec2_4 = DecoderBlock(160 + 48, 48)
        self.skip_dec1_3 = DecoderBlock(88 + 32, 32)
        self.skip_dec1_4 = DecoderBlock(160 + 32, 32)
    ...
    def forward(self, x: torch.Tensor):
        input_size = x.shape[2:]  # keep dynamic (no hardcoded 512)
        ...
        # Dense skips (same pattern as your model)
        x1_3 = self.up3(x0_4)
