...
"""
VanillaSegMate - Baseline Model (No Attention)
==============================================
This is the base architecture without any attention mechanisms.
Used as a baseline to demonstrate the value of SE+CBAM attention.

Changes from CustomUNetPlusPlus:
- All decoder blocks use 'none' (no attention)
- All skip connection blocks use 'none' (no attention)
- CBAM and SE modules kept for reference but not used

This ablation establishes the baseline performance of the
asymmetric encoder-decoder UNet++ structure with 3 prediction heads.
"""
...
# --- Main Vanilla SegMate (No Attention Baseline) ---
class VanillaSegMate(nn.Module):
    """
    Vanilla SegMate model without any attention mechanisms.

    This baseline model has the same architecture as CustomUNetPlusPlus
    but without CBAM on decoder blocks and without SE on skip connections.

    Used to establish baseline performance and demonstrate the value
    of the SE+CBAM attention combination.
    """
    def __init__(self, num_classes=9, in_channels=1, deep_supervision=False):
        super(VanillaSegMate, self).__init__()
        ...
        # Decoder path - NO ATTENTION (was CBAM in original)
        ...
        # Dense skip connections - NO ATTENTION (was SE in original)
        self.skip_dec3_4 = DecoderBlock(88 + 88, 88)
        self.skip_dec2_3 = DecoderBlock(48 + 48, 48)
        self.skip_dec1_2 = DecoderBlock(56, 32)
